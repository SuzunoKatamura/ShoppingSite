...
let currentOpenedProductId = null;     

// --- HTMLの読み込みが完了してからイベントを登録する ---
document.addEventListener('DOMContentLoaded', () => {

	// ここで1回だけ要素を取得しておく（毎回探さないので動きも軽くなります）
	const header = document.querySelector('.site-header');
	const toTopBtn = document.querySelector('.to-top-btn'); // CSSに合わせてクラス名で取得

	// 要素がちゃんと存在するかデバッグ用にコンソールに出力（確認したら消してOKです）
	console.log('ヘッダー要素:', header);
	console.log('トップボタン要素:', toTopBtn);

	// スクロールイベントの登録
	window.addEventListener('scroll', () => {
		// iOSなどのバウンド対策
		const currentScrollY = Math.max(0, window.scrollY); 
		const scrollDiff = currentScrollY - oldScrollPosition;
		...
		if (header) {
			if (currentScrollY <= 100) {
				...
				header.classList.add('hide'); // 下スクロールで隠す
			} else if (scrollDiff < -10) {
				header.classList.remove('hide'); // 上スクロールで出す
			}
		}

		// 2. ページトップ戻りボタンの出し入れ処理
		if (toTopBtn) {
...
// 「カートに保存する」ボタンが押されたときに動く関数
function addToCart() {
	//  今モーダルで開いている商品のID（すでに保存されているもの）を取得
	const productId = currentOpenedProductId;

	//  そのIDを使って、memoryData から「名前」と「価格」を引っ張ってくる
	const data = memoryData[productId];
	
	if (!data) {
		alert("商品のデータが見つかりません。");
		return;
	}

	//  サーブレットのURLを組み立てて、画面をガツンと遷移させる！
	// URLの後ろに「?productId=〇〇&name=〇〇&price=〇〇」の形でデータをくっつけます
	window.location.href = "/ShoppingSite/cart/cart-add" 
		+ "?productId=" + encodeURIComponent(data.id)
